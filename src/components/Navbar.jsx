...
const Navbar = () => {
  ...
  // Get logo source - using logo.png
  const getLogoSrc = () => {
    return '/logo.png';
  };
  ...
  return (
    <div
      className={`${isSticky ? 'sticky' : 'absolute'} top-0 left-0 z-40 flex items-center w-full bg-transparent ud-header`}
    >
      <div className="container px-4 mx-auto">
        <div className="relative flex items-center justify-between -mx-4">
          <div className="max-w-full px-4 w-24">
            <Link to="/" className="block w-full py-5 navbar-logo">
              <img
                src={getLogoSrc()}
                alt="logo"
                className="w-full max-w-[80px] header-logo"
              />
            </Link>
          </div>
          <div className="flex items-center justify-between w-full px-4">
            <div>
              ...
              >
                <span
                  className={`relative my-[6px] block h-[2px] w-[30px] ${isHomePage ? 'bg-white' : 'bg-dark dark:bg-white'}`}
                ></span>
                <span
                  className={`relative my-[6px] block h-[2px] w-[30px] ${isHomePage ? 'bg-white' : 'bg-dark dark:bg-white'}`}
                ></span>
              ...
              <nav
                id="navbarCollapse"
                className={`absolute right-4 top-full ${isMenuOpen ? 'block' : 'hidden'} w-full max-w-[250px] rounded-lg bg-white py-5 shadow-lg dark:bg-dark-2 lg:static lg:block lg:w-full lg:max-w-full lg:bg-transparent lg:px-4 lg:py-0 lg:shadow-none dark:lg:bg-transparent xl:px-6`}
              >
                <ul className="blcok lg:flex 2xl:ml-20">
                  <li className="relative group">
                    <Link
                      to="/"
                      className={`flex py-2 mx-8 text-base font-medium text-dark group-hover:text-primary dark:text-white lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 ${isHomePage ? 'lg:text-[#1E40AF] lg:group-hover:text-[#1E40AF] lg:group-hover:opacity-70' : 'lg:text-body-color dark:lg:text-dark-6'} ${
                        isActive('/') ? 'lg:border-b-2 lg:border-primary' : ''
                      }`}
                    >
                      Trang chủ
                    </Link>
                  ...
                  <li className="relative group">
                    <Link
                      to="/diary"
                      className={`flex py-2 mx-8 text-base font-medium text-dark group-hover:text-primary dark:text-white lg:ml-7 lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 ${isHomePage ? 'lg:text-[#1E40AF] lg:group-hover:text-[#1E40AF] lg:group-hover:opacity-70' : 'lg:text-body-color dark:lg:text-dark-6'} xl:ml-10 ${
                        isActive('/diary') ? 'lg:border-b-2 lg:border-primary' : ''
                      }`}
                    >
                      Nhật Ký
                    </Link>
                    ...
                    <>
                      <li className="relative group">
                        <Link
                          to="/dashboard"
                          className={`flex py-2 mx-8 text-base font-medium text-dark group-hover:text-primary dark:text-white lg:ml-7 lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 ${isHomePage ? 'lg:text-[#1E40AF] lg:group-hover:text-[#1E40AF] lg:group-hover:opacity-70' : 'lg:text-body-color dark:lg:text-dark-6'} xl:ml-10 ${
                            isActive('/dashboard') ? 'lg:border-b-2 lg:border-primary' : ''
                          }`}
                        >
                          Thống kê
                        </Link>
                      ...
                      <li className="relative group">
                        <Link
                          to="/calendar"
                          className={`flex py-2 mx-8 text-base font-medium text-dark group-hover:text-primary dark:text-white lg:ml-7 lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 ${isHomePage ? 'lg:text-[#1E40AF] lg:group-hover:text-[#1E40AF] lg:group-hover:opacity-70' : 'lg:text-body-color dark:lg:text-dark-6'} xl:ml-10 ${
                            isActive('/calendar') ? 'lg:border-b-2 lg:border-primary' : ''
                          }`}
                        >
                          Lịch
                        </Link>
                      ...
                      <li className="relative group">
                        <Link
                          to="/profile"
                          className={`flex py-2 mx-8 text-base font-medium text-dark group-hover:text-primary dark:text-white lg:ml-7 lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 ${isHomePage ? 'lg:text-[#1E40AF] lg:group-hover:text-[#1E40AF] lg:group-hover:opacity-70' : 'lg:text-body-color dark:lg:text-dark-6'} xl:ml-10 ${
                            isActive('/profile') ? 'lg:border-b-2 lg:border-primary' : ''
                          }`}
                        >
                          Hồ sơ
                        </Link>
            ...
            <div className="flex items-center justify-end pr-16 lg:pr-0">
              ...
              >
                ...
                />
                <span className={`block ${isHomePage ? 'text-[#1E40AF]' : 'text-dark'} dark:hidden`}>
                  <svg
                ...
                <span className={`hidden ${isHomePage ? 'text-[#1E40AF]' : 'text-dark'} dark:block dark:text-white`}>
                  <svg
                    className="fill-current"
                    width="24"
                    height="24"
              ...
              <div className="hidden sm:flex items-center gap-4">
                {user ? (
                  <>
                    <Link
                      to="/memory-lane"
                      className={`px-4 py-2 text-base font-medium ${isHomePage ? 'text-[#1E40AF] hover:opacity-70' : 'text-dark dark:text-white hover:text-primary'} transition`}
                      title="Nhớ lại ngày này"
                    >
                      📅 Nhớ lại
                    </Link>
                    <Link
                  ...
                  <>
                    <Link
                      to="/signin"
                      className={`loginBtn px-[22px] py-2 text-base font-medium ${isHomePage ? 'text-[#1E40AF] hover:opacity-70' : 'text-dark dark:text-white hover:text-primary'} transition`}
                    >
                      Đăng nhập
                    </Link>
                    <Link
                      to="/signup"
                      className={`px-6 py-2 text-base font-medium ${isHomePage ? 'text-[#1E40AF] bg-white/20 signUpBtn hover:bg-white/100 hover:text-dark' : 'text-[#1E40AF] bg-primary hover:bg-blue-dark'} duration-300 ease-in-out rounded-md transition`}
                    >
                      Đăng ký
                    </Link>
                  </>
